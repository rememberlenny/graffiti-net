# How to provision a new environment for graffiti-net
...
* Enter domains to secure (for example): `graffiti-net.org`
* Enter webroot path: `/home/deployer/apps/graffiti_net/shared/public/`

The `.pem` files created by `certbot-auto` need to be linked so Nginx can see them:

```
ln -s /etc/letsencrypt/live/graffiti-net.org/fullchain.pem /etc/ssl/graffiti_net.crt
ln -s /etc/letsencrypt/live/graffiti-net.org/privkey.pem /etc/ssl/graffiti_net.key
```
...
```
/opt/certbot/certbot-auto renew --webroot-path /home/deployer/apps/graffiti_net/shared/public/ --dry-run
```

Finally, add this to the root crontab to enable auto-renewal:

```
7 8,20 * * * /opt/certbot/certbot-auto renew --quiet --no-self-upgrade --webroot-path /home/deployer/apps/graffiti_net/shared/public/
```
